...
const create = ({ nombre, apellidos, direccion, email, edad, sexo, cuota, fecha_nacimiento, dni, profesor }) => {
    return new Promise((resolve, reject) => {
        db.query('insert into clientes (nombre, apellidos, direccion, email, edad, sexo, cuota, fecha_nacimiento, dni, profesor) values (?,?,?,?,?,?,?,?,?,?)', [nombre, apellidos, direccion, email, edad, sexo, cuota, fecha_nacimiento, dni, profesor]),
            (err, result) => {
                console.log(err)
                if (err) reject(err);
                resolve(result);
